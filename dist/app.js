...
const plans={tv:['电视移动','同一台 C02 的三处停放；实线去卧室，虚线去用餐位。先移开边几和餐桌，按整机方向过门；不是机器人轨迹。'],layout:['全屋布局','布局图 / 柜号与收纳图对应；V01–V06 为效果图视角，L 形为固定沙发与贵妃榻。'],life:['生活动线','绿色线表示典型生活路径：回家、用餐、洗浴与休息。为示意连接，非所有人的唯一走法。'],robot:['机器人清扫','绿色为主要连接；棕色虚线为有条件开放的卫浴干区。淋浴、设备间和室外不纳入清扫；路线不等于实际算法轨迹。']};
...
function selectPlan(tab){
 ...
 panel.setAttribute('aria-labelledby',tab.id);link.href=`assets/plan-${key}-v6.svg`;link.dataset.caption=`安全屋概念平面 / ${plans[key][0]}`;img.src=link.getAttribute('href');img.alt=link.dataset.caption;document.querySelector('#plan-caption').textContent=plans[key][1];
}
