...
def living_section(facts):
 return '<div class="practical-detail"><p class="eyebrow">T01 / 用餐与随手放置</p><h4>软的负责坐，硬的负责放。</h4>'+facts([
 ('杯子有稳定的台面','保留沙发侧边的硬质边几，放水杯、手机和遥控器。它服务于随手取放，不承担整顿外卖。'),
 ('餐桌靠近坐姿','采用约 650 × 400 mm 易擦洗硬质桌面，可调高度、可移动并能锁定。图中为用餐状态；按沙发底部净空核对底座是否真正能插入。'),
 ('用完就把通道还回来','中央不再摆布艺脚踏。吃完移开小桌后再清扫，餐桌轮脚、侧桌底座也要纳入机器人避障与人工补扫范围。')])+'<p class="caption">用餐桌优先核对抗倾覆、锁定稳定性、桌缘与身体距离，再决定高度；效果图不代替家具实测。升降或移动时先移走热饮和餐具。</p></div>'
