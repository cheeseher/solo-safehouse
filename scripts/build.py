...
from recliner import section as living_section,render as render_recliner
...
for mode in ['layout','life','robot']:
 ...
 out+='</g>'+txt(60,1098,'概念布局，非实测户型。蓝箭头为视角；棕虚线为沙发活动范围；路线表示连接，非导航轨迹。',15,'start')+'</svg>'
 (A/f'plan-{mode}-v4.svg').write_text(out)
...
render_recliner(svg_start,rect,txt,line,A)

def photo(name,alt,cls='',eager=False):
 name={'living':'sofa-recline','study':'study-v2','kitchen':'kitchen-v4','bathroom':'bathroom-v4'}.get(name,name)
...
def diagram(name,alt):
 name=name+'-v4' if name.startswith('plan-') else name
...
rooms=[
('living','01','客厅','把时间，留给一部电影。','大屏、能窝进去的沙发、顺手拿到的水。放松不需要复杂的场景，也不需要填满每一面墙。',[('98 英寸电视','电视在图左侧墙，沙发在图右侧；效果图朝露台看，因此画面左右相反。以电视＋NAS 为核心。按最终电视尺寸、坐姿与现场距离试看，不把普通观影升级成家庭影院。'),('宽座布艺电动沙发','收起、抬腿、后仰分开表达。连续躺卧支撑长度目标约 1850mm，按 170cm 身高试躺；具体角度与动作以选定型号为准。'),('清扫有明确边界','悬浮影音柜底部预留 180mm。电动沙发机构区不纳入机器人清扫，整块活动范围设禁入；通路与地面供电线路单独核对。')],'C02'),
...
for image,num,name,title,desc,rows,cid in rooms:
 detail=desk_section(diagram,facts) if image=='study' else living_section(photo,diagram,facts) if image=='living' else ''
