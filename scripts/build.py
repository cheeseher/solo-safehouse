...
import json,html
from practical_details import render_desktop,desk_section,living_section
...
for mode in ['layout','life','robot']:
 ...
 for x,y,w,h,fill in [(3420,5000,1100,2800,'#bbc4b7'),(3190,7090,400,650,'#d5cbb6'),(9000,1700,1800,900,'#d5cbb6'),(8260,6320,1980,2180,'#e3e3d7'),(8350,6440,1800,2000,'#faf9f2'),(7940,2920,1200,1200,'#bed4ce'),(10200,4320,800,1700,'#f6f8f1'),(8040,4420,850,660,'#f6f8f1')]:out+=R(x,y,w,h,fill)
 out+=f'<circle cx="3890" cy="8190" r="210" fill="#d8d4c8" stroke="#8e998e" stroke-width="14"/>'+T(2930,7440,'餐桌',130)
 ...
 (A/f'plan-{mode}-v2.svg').write_text(out)
...
def photo(name,alt,cls='',eager=False):
 name={'living':'living-v2','study':'study-v2'}.get(name,name)
...
def diagram(name,alt):
 name=name+'-v2' if name.startswith('plan-') else name
...
rooms=[
('living','01','客厅','把时间，留给一部电影。','大屏、能窝进去的沙发、顺手拿到的水。放松不需要复杂的场景，也不需要填满每一面墙。',[('98 英寸电视','以电视＋NAS 为核心。按最终电视尺寸、坐姿与现场距离试看，不把普通观影升级成家庭影院。'),('深座布艺沙发','概念尺寸 2800 × 1100 mm。取消中央脚踏，改用可拉近的硬质小餐桌；直坐时可加靠垫，偶尔留宿需核对有效躺卧长度。'),('可清扫的底部','悬浮影音柜底部预留 180 mm。沙发支脚和横撑也要一起检查，外观悬空不等于机器人能进入。')],'C02'),
...
for image,num,name,title,desc,rows,cid in rooms:
 detail=desk_section(diagram,facts) if image=='study' else living_section(facts) if image=='living' else ''
 roomhtml+=f'<article class="room" id="room-{image}"><div class="room-intro"><p class="eyebrow">{num} / {name}</p><h3>{title}</h3><p class="lede">{desc}</p></div>{photo(image,name+"空间概念效果图；尺寸与结构以图纸为准")}<div class="room-bottom">{facts(rows)}<a class="textlink" href="#cab-{cid}">查看{name}柜体分层 <span>↗</span></a>{detail}</div></article>'
notes={
