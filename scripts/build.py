...
for mode in ['layout','life','robot']:
 ...
 for x,y,w,h,fill in [(3420,5000,1100,2800,'#bbc4b7'),(2720,6570,800,850,'#bbc4b7'),(9000,1700,1800,900,'#d5cbb6'),(8260,6320,1980,2180,'#e3e3d7'),(8350,6440,1800,2000,'#faf9f2'),(7940,2920,1200,1200,'#bed4ce'),(10200,4320,800,1700,'#f6f8f1'),(8040,4420,850,660,'#f6f8f1')]:out+=R(x,y,w,h,fill)
 out+=T(8510,3560,'淋浴',150)+T(10600,5100,'浴缸',150)+T(8450,4800,'马桶',150)
 ...
 routes=[]
 if mode=='robot':
  routes=[[(7100,4130),(6050,4130),(4950,4130),(1900,4130),(1900,7100)],[(6050,4130),(6050,1950),(7090,1950),(7990,1950)],[(6050,4130),(6050,5570),(7190,5570),(7190,7470)],[(1900,4130),(1900,2700)],[(6050,2400),(4750,2400),(4750,3280)]]
 ...
 elif mode=='life':routes=[[(6050,0),(6050,2400),(4750,2400),(4750,4030),(1900,4030),(1900,2700)],[(1900,4200),(1900,6900)],[(6050,5000),(6050,5570),(7300,5570),(9450,5570)],[(7200,5570),(7200,7450)],[(6050,1950),(8100,1950)]]
 ...
 out+='</g>'+txt(60,1098,'概念布局，非实测户型。路线表示空间连接，不代表机器人的实际导航轨迹。',15,'start')+'</svg>'
 (A/f'plan-{mode}.svg').write_text(out)
...
def diagram(name,alt):return f'<a class="diagram" href="assets/{name}.svg" data-lightbox data-caption="{esc(alt)}"><img src="assets/{name}.svg" alt="{esc(alt)}" loading="lazy"><span class="expand">放大图纸 ↗</span></a>'
def facts(rows):return '<dl class="facts">'+''.join(f'<div><dt>{a}</dt><dd>{b}</dd></div>' for a,b in rows)+'</dl>'
rooms=[
('living','01','客厅','把时间，留给一部电影。','大屏、能窝进去的沙发、顺手拿到的水。放松不需要复杂的场景，也不需要填满每一面墙。',[('98 英寸电视','以电视＋NAS 为核心。按最终电视尺寸、坐姿与现场距离试看，不把普通观影升级成家庭影院。'),('深座布艺沙发','概念尺寸 2800 × 1100 mm，搭配活动脚踏。深座供躺靠，直坐可加靠垫；偶尔留宿需核对有效躺卧长度。'),('可清扫的底部','悬浮影音柜底部预留 180 mm。沙发支脚和横撑也要一起检查，外观悬空不等于机器人能进入。')],'C02'),
('study','02','工作室','桌面很简单。背后很完整。','桌面只留下正在使用的设备。文件、配件、打印机和网络设备各有位置，工作开始时不用先整理半小时。',[('1800 × 900 mm 工作台','预留双显示器与键鼠空间。线槽放在桌后可触达处，坐姿净空不被抽屉和主机挤占。'),('原装充电，更容易替换','高频快充保留 AC 插座与原装充电器，不把不可升级的固定 USB 当作唯一方案。'),('打印机有自己的使用位置','放在可移出的独立小车上，使用时移到通风位置。进出纸、开盖、换耗材所需空间一起预留。')],'C03'),
...
for image,num,name,title,desc,rows,cid in rooms:
 roomhtml+=f'<article class="room" id="room-{image}"><div class="room-intro"><p class="eyebrow">{num} / {name}</p><h3>{title}</h3><p class="lede">{desc}</p></div>{photo(image,name+"空间概念效果图；尺寸与结构以图纸为准")}<div class="room-bottom">{facts(rows)}<a class="textlink" href="#cab-{cid}">查看{name}柜体分层 <span>↗</span></a></div></article>'
notes={
'C01':('出门与回家的动作，在同一处结束。','每日鞋位与随手放置台靠近入口；背包就近归位。快递开箱工具集中存放，包装及时清走，顶部只放低频用品。'),
'C02':('服务观影，也服务清扫。','遥控器、手柄和播放器分区。运行设备的散热与线材检修保持可达；底部 180 mm 离地仅为柜体方案，不代表所有扫地机都适配。'),
...
'C06':('接水与泡茶，一次准备好。','水杯、茶具和茶叶就近放置；净水阀门独立检修。接水台做好接滴与清洁，避免水路经过难以维护的封闭空间。'),
...
<a class="skip" href="#main">跳到正文</a><header class="site-header"><a class="wordmark" href="#top" aria-label="个人安全屋首页">SOLO<span>个人安全屋</span></a><nav aria-label="主要章节"><a href="#idea">理念</a><a href="#plan">布局</a><a href="#spaces">空间</a><a href="#storage">收纳</a><a href="#practice">落地</a></nav></header>
...
<section class="plan-section section" id="plan"><div class="section-label"><span>02 / 全屋布局</span><span>EVERYDAY, CONNECTED.</span></div><div class="section-head"><h2>先安排生活，<br>再划分房间。</h2><p>以约 98.3㎡ 室内净地面概念方案为载体。休息与工作分区，家政靠近衣物循环，设备独立安放。重要的不是面积数字，而是每段路为什么存在。</p></div><div class="plan-workspace"><div class="plan-main"><div class="tabs" role="tablist" aria-label="平面图层"><button id="tab-layout" role="tab" aria-controls="plan-panel" aria-selected="true" data-plan="layout">全屋布局</button><button id="tab-life" role="tab" aria-controls="plan-panel" aria-selected="false" tabindex="-1" data-plan="life">生活动线</button><button id="tab-robot" role="tab" aria-controls="plan-panel" aria-selected="false" tabindex="-1" data-plan="robot">机器人清扫</button></div><div id="plan-panel" role="tabpanel" aria-labelledby="tab-layout">{diagram('plan-layout','安全屋概念平面图：客厅、厨房、工作室、卧室、卫浴和设备家政位置')}</div><p id="plan-caption" class="caption" aria-live="polite">布局图 / 柜号与下方收纳图一一对应，点图可放大。</p></div><aside class="plan-notes"><span class="big-number">98.3<small>㎡</small></span><p>概念室内净地面<br>包含柜体占地 · 露台另计</p><div class="rule"></div><h3>生活的三条短路径</h3><ol><li><strong>回家 → 放下 → 放松</strong><span>鞋、包与快递工具在玄关归位，外卖顺路进入加热与用餐区域。</span></li><li><strong>脱衣 → 洗烘 → 衣柜</strong><span>脏衣与干净待归位衣物分开，家政靠近卧室，减少搬运。</span></li><li><strong>基站 → 清扫 → 回充</strong><span>设备和淋浴区不纳入自动清扫；卫浴干区仅在地面干燥、无积水时开放。</span></li></ol><p class="caption">内包络 11.0 × 9.2m，扣除概念内墙约 2.94㎡。不等同于房产证建筑面积，也不表示必须购买这个面积。</p></aside></div><noscript><p>图层切换需要 JavaScript。也可以直接打开 <a href="assets/plan-life.svg">生活动线图</a> 或 <a href="assets/plan-robot.svg">清扫连接图</a>。</p></noscript></section>
...
print('Generated HTML, 11 cabinet diagrams, 3 drawers, 3 plans and integrated dryer diagram.')
