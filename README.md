...
客厅改为布艺电动伸展沙发，提供收起、抬腿与后仰三张同视角图，支持手动切换和可暂停的循环演示。关闭 JavaScript 时展示全部状态。有效支撑长度目标 1850mm，最大活动范围暂按 2800 × 2150mm 预留；不承诺具体型号动作参数。机构范围不纳入机器人清扫，餐桌独立落地并停回厨房 C06 右段。

## 图纸与效果图对应修订

- `scripts/spatial.py` 统一沙发、其他家具、路线和 V01–V06 视角坐标。三个平面图均更新至 v4，补上外窗、露台入口、C07M 镜柜标注与沙发活动范围。
- 修正工作台朝向与窗边位置；厨房图移除错误前景沙发与右侧操作台；卫浴图统一淋浴、单台盆、马桶和浴缸位置及柜门分格。
- C06 右段改为700mm名义宽开放停放位，左段500mm、茶具段900mm；净尺寸按板厚与桌凳实物复核。
- `scripts/recliner.py` 生成沙发章节与占位图；`scripts/skirting_details.py` 生成收边章节与结构图。
